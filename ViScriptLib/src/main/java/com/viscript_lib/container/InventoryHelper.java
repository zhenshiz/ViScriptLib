...
public class InventoryHelper implements IContainerHelper {
    ...
    public long getItemStackCount(ServerPlayer player, ItemStack item,
        ...
        long count = ItemUtil.getItemCountByContainer(player.getInventory(), item, compareMode, components);
        count = ItemUtil.saturatedAdd(count, ItemUtil.getItemCountByContainer(
                player.inventoryMenu.getCraftSlots(), item, compareMode, components
        ));
        ...
        count = ItemUtil.saturatedAdd(count, ItemUtil.getItemCountByContainer(
                player.getEnderChestInventory(), item, compareMode, components
        ));
        return count;
    }
    ...
    public long removeItemStackByCount(ServerPlayer player, ItemStack item, long count,
        ...
        count = ItemUtil.removeItemByContainer(
                player.getInventory(), item, count, compareMode, components
        );
        count = ItemUtil.removeItemByContainer(player.inventoryMenu.getCraftSlots(), item, count, compareMode, components);
        ItemStack carried = player.containerMenu.getCarried();
        if (count > 0L && ItemUtil.isSameItem(carried, item, compareMode, components)) {
            int removed = (int) Math.min(count, carried.getCount());
