...
public final class MissingItemStackRecovery {
     ...
     *
     * <p>通过 LDLib2 默认访问器持久化的物品栈会在恢复作用域中
    ...
    public static final Codec<ItemStack> CODEC = new Codec<>() {
        ...
        public <T> DataResult<Pair<ItemStack, T>> decode(DynamicOps<T> ops, T input) {
            ...
            return recovered.<DataResult<Pair<ItemStack, T>>>map(stack -> DataResult.success(Pair.of(stack, input))).orElseGet(() -> LDLibExtraCodecs.ITEM_STACK.decode(ops, input));
        }
    ...
    public static ItemStack createBarrierPlaceholder(ResourceLocation itemId) {
        ...
        placeholder.setHoverName(Component.literal(itemId.toString()).withStyle(ChatFormatting.RED));
        CompoundTag tag = ItemUtil.getNbt(placeholder);
        CompoundTag display = tag.getCompound("display");
        ListTag lore = new ListTag();
        lore.add(StringTag.valueOf(Component.Serializer.toJson(Component.translatable("viscript_lib.missing_item.placeholder_lore").withStyle(style -> style
