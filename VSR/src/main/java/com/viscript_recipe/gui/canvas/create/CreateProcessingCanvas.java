...
public class CreateProcessingCanvas extends FluidRecipeCanvas<CreateProcessingRecipeData> {
    ...
    public void load() {
        ...
        if (ingredients != null) {
            ...
            if (supportsCountedItemInputs(kind)) {
                ...
                for (var ingredient : ingredients) {
                    ...
                    loadIngredientSlot(visualIndex, normalizedIngredient);
        ...
        for (int i = 0; i < Math.min(Math.min(CREATE_MAX_FLUID_INPUTS, kind.maxFluidInputs()), fluidInputs.size()); i++) {
            ...
            setVisualFluidInput(i, input == null ? FluidIngredientData.empty() : input.copy());
        }
        ...
        for (int i = 0; i < Math.min(Math.min(CREATE_MAX_FLUID_OUTPUTS, kind.maxFluidOutputs()), fluidOutputs.size()); i++) {
            ...
            setVisualFluidOutput(i, output == null ? FluidStack.EMPTY : output.copy());
        }
    }
    ...
    public void save() {
        ...
        var visualIngredientCount = createVisibleItemInputCapacity(kind);
        var countedInputs = supportsCountedItemInputs(kind);
        var remainingIngredientWeight = visualIngredientCount;
        ...
        for (int i = 0; i < visualFluidInputCount; i++) {
            if (!getVisualFluidInput(i).isEmpty()) lastFluidInputIndex = i;
        }
        var fluidIngredients = new ArrayList<>(Arrays.asList(visualFluidInputs).subList(0, lastFluidInputIndex + 1));
            ...
            content.addChild(RecipeEditorUi.fieldGroup("viscript_recipe.config.create.heat_requirement",
                    RecipeEditorUi.selector(
                            ...
                            CreateHeatCondition::displayName, value -> {
                                data.setHeatRequirement(value); navigationView.reloadCanvas();
                            }
                    )));
        }
        if (kind.keepHeldItemAllowed()) {
            content.addChild(RecipeEditorUi.fieldGroup("viscript_recipe.config.create.keep_held_item",
                    new Switch().setOn(data.isKeepHeldItem(), false).setOnSwitchChanged(data::setKeepHeldItem)));
        }
    }
    ...
    public void buildResultProperties(UIElement content) {
        ...
        if (outputWithChance()) {
            content.addChild(RecipeEditorUi.fieldGroup("viscript_recipe.config.create.output.chance",
    ...
    public boolean outputWithChance() {
        var kind = getCreateProcessingKind();
        return kind != null && kind != CreateProcessingKind.BLOCK_CUTTING
                ...
                (index, row, col) -> {
                    ...
                    if (index != 0) { // 全部操作第一个格子
                        ...
                        slot.addEventListener(UIEvents.MOUSE_DOWN, event -> {
                            ...
                            navigationView.setSlotSelection(SlotSelection.ingredient(0));
                        });
                    }
                    return RecipeGridFactory.slotCell(slot, SLOT_SIZE);
                }
